...
class AccountAction extends Action
{
    ...
    public function doAuthenticate()
    {
        ...
        //2017.9.8添加统一信用号信息取得
        $data['company_tyidcard'] = trim(t($_POST['company_tyidcard']));
        ...
        if (preg_match($Regx2, $data['idcard']) == 0 || preg_match($Regx2, substr($data['idcard'], 0, 17)) == 0 || strlen($data['idcard']) !== 18) {
            exit('请输入正确的身份证号码');
        }

        //2017.9.8添加统一信用号判断
        if (preg_match($Regx4, $data['company_tyidcard']) == 0 || strlen($data['company_tyidcard']) !== 18) {
            exit('请输入正确的统一信用号');
        }
